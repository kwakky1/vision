...
import { isValidString } from "./ValidateObject";
...
export function Sprintf(format, ...optionalParams) {
  if (isValidString(format) && (format.indexOf('%') > -1)) {
    try {
      return sprintf(format, ...optionalParams);      
    } catch (error) {
      console.log("Sprintf Error!!! ", format)
      return format
    }
  }
  return format
